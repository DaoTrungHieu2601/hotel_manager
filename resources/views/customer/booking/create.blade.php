<x-guest-hotel-layout :title="__('Xác nhận đặt phòng')">
    <div class="mx-auto max-w-lg px-4 py-12">
        <div class="rounded-3xl bg-white p-8 font-sans text-slate-900 shadow-xl ring-1 ring-stone-200">
            <h1 class="font-display text-2xl font-bold text-amber-950">{{ __('Xác nhận đặt phòng') }}</h1>
            <dl class="mt-6 space-y-2 text-sm">
                <div class="flex justify-between gap-4"><dt class="shrink-0 text-slate-600">{{ __('Loại phòng') }}</dt><dd class="text-right font-medium text-slate-900">{{ $roomType->name }}</dd></div>
                ...
                <div class="flex justify-between gap-4 text-lg font-bold text-amber-900"><dt class="shrink-0">{{ __('Giá tham khảo / đêm') }}</dt><dd class="text-right tabular-nums">{{ number_format($roomType->default_price, 0, ',', '.') }} ₫</dd></div>
            ...
            @endif
            <form method="post" action="{{ route('customer.bookings.store') }}" class="mt-8 space-y-4">
                @csrf
